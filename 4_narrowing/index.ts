...
function userGreeting(user: object) {
  if (user instanceof SuperUser) {
    console.log(
      `Olá ${user.name}, gostaria de ter acesso a todos os dados do sistema?`
    );
...
function verifyReview(note: number | boolean) {
  if (typeof note === "number") {
    console.log("Obrigado por ter deixado sua review!");
  } else {
    console.log(
      "na próxima oportunidade deixe sua avaliação, sua participação é sempre bem-vinda!"
    );
  }
}
